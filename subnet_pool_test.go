...
func TestSubnetPoolCreateOrUpdate(t *testing.T) {
	...
	Expect(m["method"].(string)).To(Equal("subnet_pool_get"))
	...
	Expect(m["method"].(string)).To(Equal("subnet_pool_create"))

	params = (m["params"].([]interface{}))

	Expect(params[0].(map[string]interface{})["subnet_pool_id"].(float64)).To(Equal(100.0))
}

func TestSubnetPoolDeleteForApply(t *testing.T) {
	...
	Expect(m["method"].(string)).To(Equal("subnet_pool_delete"))
