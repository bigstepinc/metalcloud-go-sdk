...
var tpl = `// DO NOT EDIT: this file is automatically generated by docgen
package {{ .Package }}
import (
...
func main() {
	...
	abs, err := in(os.Args[2])
	if err != nil {
		log.Fatal(err)
	}

	if len(os.Args) != 6 {
		log.Fatalf("expected 4 args, got %d. Remember you need a '-' as first argument.", len(os.Args)-2)
	}
	...
	structsToUse := os.Args[4]
	structsToUseArr := strings.Split(structsToUse, ",")
	structsFiltered := []*structType{}
	for _, s := range structs {
		for _, ss := range structsToUseArr {
			if ss == s.name {
				structsFiltered = append(structsFiltered, s)
			}
		}
	}

	if len(structsFiltered) == 0 {
		log.Fatalf("failed to find the specific types that were provided (%s) in %s", structsToUse, abs)
	}

	structs = structsFiltered
	...
	base_no_ext := strings.TrimSuffix(base, ".go")
	fname := filepath.Join(os.Args[3], fmt.Sprintf("%s_doc.go", base_no_ext))
	doc.File = os.Args[3]
	//fmt.Printf("writing docs to: %s\n", os.Args[3])
	render(doc, fname)
}
